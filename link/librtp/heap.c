...
child->value = temp;               \
temp = parent->immutable;              \
parent->immutable = child->immutable;      \
child->immutable = temp;               \
}
...
int heap_min(heap* h, heap_entry **pEntry) {
...
heap_entry* heap_insert(heap *h, void* key, void* value) {
...
int heap_delmin(heap* h, heap_entry **pEntry) {
