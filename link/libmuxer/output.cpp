...
int AvEncoder::Init(IN const std::shared_ptr<MediaFrame>& _pFrame)
{
        ...
        case STREAM_VIDEO:
        {
                // hardware encoding first, otherwise degrading accordingly
                std::vector<std::string> videoCodecs{"h264_qsv", "h264_nvenc", "libx264"};
                codecs = videoCodecs;
                break;
        }
        default:
        ...
        // open encoder and return results
        auto openEnc = [&](AVCodec* _pCodec, std::string& _name) -> bool {
                // initiate AVCodecContext
                pAvEncoderContext_ = avcodec_alloc_context3(_pCodec);
                if (pAvEncoderContext_ == nullptr) {
                        Error("could not allocate AV codec context for encoder");
                        return false;
                }

                std::shared_ptr<MediaFrame> pFrame = _pFrame;

                if (_name.compare("h264_qsv") == 0) {
                        // special case for intel QSV which needs NV12 pictures
                        // send a fake frame containing identical attrs to qsv preset
                        pFrame = std::make_shared<MediaFrame>(*_pFrame);
                        pFrame->AvFrame()->format = AV_PIX_FMT_NV12; // only support NV12
                }
                ...
                // open encoder
                if (avcodec_open2(pAvEncoderContext_, _pCodec, nullptr) < 0) {
                        Error("could not open encoder");
                        avcodec_free_context(&pAvEncoderContext_);
                        pAvEncoderContext_ = nullptr;
                        return false;
                } else {
                        Info("open encoder: stream=%d, codec=%d", pFrame->Stream(), pFrame->Codec());
                        bIsEncoderAvailable_ = true;
                        return true;
                }
        };

        auto result = std::find_if(codecs.begin(), codecs.end(), [&openEnc](std::string _name) {
                        Info("find encoder: %s", _name.c_str());
                        AVCodec* pAvCodec = avcodec_find_encoder_by_name(_name.c_str());
                        if (pAvCodec == nullptr) {
                                Error("could not find encoder: %s", _name.c_str());
                                return false;
                        }
                        return openEnc(pAvCodec, _name);
                });
        if (result == std::end(codecs)) {
                return -1;
        }
...
int AvEncoder::PresetAac(IN const std::shared_ptr<MediaFrame>& _pFrame)
{
        ...
        Info("default aac preset: sample_fmt=s16, sample_rate=%d, channels=%d, bit_rate=%ld",
             pAvEncoderContext_->sample_rate, pAvEncoderContext_->channels,  pAvEncoderContext_->bit_rate);
...
int AvEncoder::PresetH264(IN const std::shared_ptr<MediaFrame>& _pFrame)
{
        ...
        pAvEncoderContext_->pix_fmt = static_cast<AVPixelFormat>(_pFrame->AvFrame()->format);
        ...
        int nBps = 800 * 1000; // bps
        if (nBitrate_ > 0) {
                nBps = nBitrate_ * 1000;
        }
        pAvEncoderContext_->bit_rate = nBps;
        pAvEncoderContext_->rc_max_rate = nBps;
        pAvEncoderContext_->rc_min_rate = nBps;
        pAvEncoderContext_->bit_rate_tolerance = nBps;
        pAvEncoderContext_->rc_buffer_size = nBps;
        pAvEncoderContext_->rc_initial_buffer_occupancy = pAvEncoderContext_->rc_buffer_size * 3 / 4;

        // set params
        av_opt_set(pAvEncoderContext_->priv_data, "vprofile", "high", 0);
        av_opt_set(pAvEncoderContext_->priv_data, "preset", "faster", 0);
        av_opt_set(pAvEncoderContext_->priv_data, "tune", "zerolatency", 0);

        // use rescaler if encoder requires different colorspace
        pRescaler_ = std::make_shared<VideoRescaler>(pAvEncoderContext_->width, pAvEncoderContext_->height, pAvEncoderContext_->pix_fmt);
...
int AvEncoder::EncodeH264(IN std::shared_ptr<MediaFrame>& _pFrame, IN EncoderHandlerType& _callback)
{
        ...
        // if specific colorspace is required by ecnoder, rescale the picture
        if (static_cast<AVPixelFormat>(_pFrame->AvFrame()->format) != pAvEncoderContext_->pix_fmt) {
...
std::mutex RtmpSender::librtmpLck_;

RtmpSender::RtmpSender()
{
        // lock libRTMP since this library is not thread-safe
        std::lock_guard<std::mutex> lock(librtmpLck_);
...
int RtmpSender::SendChunkSize(IN size_t _nSize)
{
        ...
        return SendPacketLarge(RTMP_PACKET_TYPE_CHUNK_SIZE, body.data(), body.size(), 0);
}
...
{
        return SendH264Packet(_buffer.data(), _buffer.size(), _packet.Pts(), false, _packet.Pts() - _packet.Dts());
}

int RtmpSender::SendH264Idr(IN const std::vector<char>& _buffer, IN const MediaPacket& _packet)
{
        return SendH264Packet(_buffer.data(), _buffer.size(), _packet.Pts(), true, _packet.Pts() - _packet.Dts());
}
...
{
        return SendPacket(RTMP_PACKET_TYPE_INFO, _packet.Data(), _packet.Size(),  _packet.Pts());
}
...
int RtmpSender::SendPacketLarge(IN unsigned int _nPacketType, IN const char* _pData,
                                IN size_t _nSize, IN size_t _nTimestamp)
{
        return SendRawPacket(_nPacketType, RTMP_PACKET_SIZE_LARGE, _pData, _nSize, _nTimestamp);
}

int RtmpSender::SendPacketMedium(IN unsigned int _nPacketType, IN const char* _pData,
                                 IN size_t _nSize, IN size_t _nTimestamp)
{
        return SendRawPacket(_nPacketType, RTMP_PACKET_SIZE_MEDIUM, _pData, _nSize, _nTimestamp);
}
                              ...
                              IN const char* _pData, IN size_t _nSize, IN size_t _nTimestamp)
{
        ...
        case RTMP_PACKET_TYPE_AUDIO:
                ...
                nCalculated = AccTimestamp(_nTimestamp, nAudioTimestamp_, nAudioSequence_);
                break;
        case RTMP_PACKET_TYPE_VIDEO:
                packet.m_nChannel = CHANNEL_VIDEO;
                nCalculated = AccTimestamp(_nTimestamp, nVideoTimestamp_, nVideoSequence_);
                break;                
        case RTMP_PACKET_TYPE_INFO:
                packet.m_nChannel = CHANNEL_META;
                nCalculated = AccTimestamp(_nTimestamp, nDataTimestamp_, nDataSequence_);
                break;
...
Rtmp::Rtmp(IN const std::string& _name)
        :Output(_name),
         muxedQ_(100)
{
...
void Rtmp::Start(IN const std::string& _url)
{
        stat.Start(1);
        ...
        auto snd = [this, _url] {
                while (bSenderExit_.load() == false) {
                        ...
                        auto encoderHook = [&](IN const std::shared_ptr<MediaPacket>& _pPacket) -> int {
                                ...
                                // output packets
                                auto nRetVal = avSender->Send(_url, _pPacket);
                                if (nRetVal == 0) {
                                        stat.OnePacket(_pPacket);
                                }
                                return nRetVal;
                        };
                        ...
                        // prevent sender reconnecting too fast
...
bool Rtmp::Push(IN const std::shared_ptr<MediaFrame>& _pFrame)
{
...
FileSink::~FileSink()
{
        if (pOutputContext_ != nullptr) {
                ...
                // close file
                if (!(pOutputContext_->oformat->flags & AVFMT_NOFILE) && pOutputContext_->pb != nullptr) {
                        // close file
                        avio_closep(&pOutputContext_->pb);
                }
...
int FileSink::Write(IN const std::string& _path, IN const std::shared_ptr<MediaPacket>& _pPacket)
{
        path_ = _path;
        ...
        if (bAudioAdded_ && bVideoAdded_) {
                ...
                // write file header
                if (bAudioAdded_ && bVideoAdded_ && WriteHeader() == false) {
                        return -2;
                }
        }
...
int FileSink::AddStream(IN const std::shared_ptr<MediaPacket>& _pPacket)
{
        ...
        if (_pPacket->AvCodecContext() == nullptr) {
                Error("file sink: source codec parameter pointer is null");
                return -2;
        }

        auto nStatus = avcodec_parameters_from_context(pStream->codecpar, _pPacket->AvCodecContext());
        if (nStatus < 0) {
...
{
        // open output file
        if (!(pOutputContext_->oformat->flags & AVFMT_NOFILE)) {
                auto nStatus = avio_open(&pOutputContext_->pb, path_.c_str(), AVIO_FLAG_WRITE);
                if (nStatus < 0) {
                        Error("file sink: could not open output file: %s: %d", path_.c_str(), nStatus);
                        return false;
                }
        }
...
File::File(IN const std::string& _name)
        :Output(_name),
         muxedQ_(100)
{
        bSinkExit_.store(false);
}
...
void File::Start(IN const std::string& _path)
{
        if (_path.empty()) {
                // no sender thread
                bSinkExit_.store(true);
        }

        auto sink = [this, _path] {
                while (bSinkExit_.load() == false) {
                        auto vEncoder = std::make_unique<AvEncoder>();
                        auto aEncoder = std::make_unique<AvEncoder>();
                        auto avSink = std::make_unique<FileSink>();
                        ...
                        auto encoderHook = [&](IN const std::shared_ptr<MediaPacket>& _pPacket) -> int {
                                if (bSinkExit_.load() == true) {
                                        return -1;
                                }
                                // write packets to file
                                return avSink->Write(_path, _pPacket);
                        };
...
bool File::Push(IN const std::shared_ptr<MediaFrame>& _pFrame)
{
        return muxedQ_.TryPush(_pFrame);
}
