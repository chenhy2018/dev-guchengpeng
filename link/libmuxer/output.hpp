...
namespace muxer
{
        ...
        private:
                ...
                std::shared_ptr<VideoRescaler> pRescaler_ = nullptr; // for rescaling
        ...
        public:
                FileSink();
                ~FileSink();
                int Write(IN const std::string& path, IN const std::shared_ptr<MediaPacket>& pPacket);
        private:
        ...
        public:
                ...
                virtual void Start(IN const std::string& url) = 0;
                virtual void Stop() = 0;
                virtual bool Push(IN const std::shared_ptr<MediaFrame>& pFrame) = 0;
        public:
                Statistic stat;
        protected:
                std::string name_;
        ...
        // Rtmp
        class Rtmp : public Output
        {
        public:
                Rtmp(IN const std::string& name);
                ~Rtmp();
                virtual void Start(IN const std::string& url);
                virtual void Stop();
                virtual bool Push(IN const std::shared_ptr<MediaFrame>& pFrame);
        protected:
                std::thread sender_;
        ...
        {
        public:
                File(IN const std::string& name);
                ~File();
                virtual void Start(IN const std::string& path);
                virtual void Stop();
                virtual bool Push(IN const std::shared_ptr<MediaFrame>& pFrame);
        protected:
                std::thread sink_;
                std::atomic<bool> bSinkExit_;
                SharedQueue<std::shared_ptr<MediaFrame>> muxedQ_;
        };
}
