...
func TestKodoSegment(t *testing.T) {
        ...
        // filename should be ts/ua_id/yyyy/mm/dd/hh/mm/ss/mmm/endts/fragment_start_ts/expiry.ts
        filename := "ts/ua1/2018/01/3/12/12/12/111/155321111/15332111222/123.ts"
        ...
        infos, err4 := model.GetSegmentTsInfo(xl, int64(1534385684000), int64(1534472084000), "testdeviceid5")
...
*/
        ...
        infos, err4 = model.GetSegmentTsInfo(xl, int64(1534848606000), int64(1534859606000), "testdeviceid10")

        //infos, err4 = model.GetSegmentTsInfo(xl, int64(1534414484000), int64(1534500884000), "testdeviceid10")
        //assert.Equal(t, err4, nil, "they should be equal")
        //assert.Equal(t, 12026, len(infos), "they should be equal")

        // filename should be seg/ua_id/yyyy/mm/dd/hh/mm/ss/mmm/endts
        infoF, markF,  errF := model.GetFragmentTsInfo(xl, 0, int64(1534385684000), int64(1534472084000), "testdeviceid5", "")
        assert.Equal(t, errF, nil, "they should be equal")
        assert.Equal(t, len(infoF), 11, "they should be equal")
        infoF, markF,  errF = model.GetFragmentTsInfo(xl, 1, int64(1534385684000), int64(1534472084000), "testdeviceid5", "")
        assert.Equal(t, errF, nil, "they should be equal")
        assert.Equal(t, len(infoF), 1, "they should be equal")
        infoF, markF,  errF = model.GetFragmentTsInfo(xl, 2, int64(1534385684000), int64(1534472084000), "testdeviceid5", markF)
        assert.Equal(t, errF, nil, "they should be equal")
        assert.Equal(t, len(infoF), 2, "they should be equal")

        infoF, markF,  errF = model.GetFragmentTsInfo(xl, 10, int64(1534385684000), int64(1534472084000), "testdeviceid5", markF)
        assert.Equal(t, errF, nil, "they should be equal")
        assert.Equal(t, len(infoF), 8, "they should be equal")

        infoF, markF,  errF = model.GetFragmentTsInfo(xl, 0, int64(1534443284000), int64(1534500884000), "testdeviceid10", "")
        assert.Equal(t, errF, nil, "they should be equal")
        assert.Equal(t, len(infoF), 9, "they should be equal")

        infoF, markF,  errF = model.GetFragmentTsInfo(xl, 1, int64(1534412096000), int64(1534729559000), "testdeviceid10", "")
        assert.Equal(t, errF, nil, "they should be equal")
        assert.Equal(t, len(infoF), 1, "they should be equal")
        xl.Infof("1 0 mark %s", markF)
        infoF, markF,  errF = model.GetFragmentTsInfo(xl, 1, int64(1534412096000), int64(1534729559000), "testdeviceid10", markF)
        assert.Equal(t, errF, nil, "they should be equal")
        assert.Equal(t, len(infoF), 1, "they should be equal")
        //assert.Equal(t, infos[0][SEGMENT_ITEM_END_TIME].(int64), int64(1534414490692), "they should be equal")
        xl.Infof("1 1 mark %s", markF)

        infoF, markF,  errF = model.GetFragmentTsInfo(xl, 5, int64(1534412096000), int64(1534729559000), "testdeviceid10", markF)
        assert.Equal(t, errF, nil, "they should be equal")
        assert.Equal(t, len(infoF), 5, "they should be equal")
        assert.Equal(t, infos[0][SEGMENT_ITEM_END_TIME].(int64), int64(1534414490692), "they should be equal")
        xl.Infof("1 2 mark %s", markF)

        infoF, markF,  errF = model.GetFragmentTsInfo(xl, 7, int64(1534412096000), int64(1534729559000), "testdeviceid10", markF)
        assert.Equal(t, errF, nil, "they should be equal")
