...
func TestXlog_Info(t *testing.T) {
	...
	assert.Equal(t, outStr[26:], " [RhQAAIfWo0-SNUwT][INFO] github.com/qiniu/xlog.v1/xlog_test.go:18: test\n")
...
func TestLogger_XgetTag(t *testing.T) {
	xl := NewDummy()
	xl2 := xl.Spawn()
	xl2.Xtag("fop")
	xtag := xl2.XgetTag()
	xl.XputTag(xtag)
	assert.Equal(t, 1, len(xl.XgetTag()))
	assert.Equal(t, "fop", xl.XgetTag()[0])
}
