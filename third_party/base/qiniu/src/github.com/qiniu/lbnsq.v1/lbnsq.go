...
func New(cfg *ProducerConfig) (cli *Client, err error) {
	err = MakeProducerConfig(cfg)
	if err != nil {
		return
	}
	cli = &Client{Cfg: cfg}
...
func (c *Client) PublishEx(xl *xlog.Logger, topic string, body []byte) (err error) {
	if xl == nil {
		xl = xlog.NewDummy()
	}
	t := time.Now()
	err = c.Publish(topic, body)
	xl.Xprof2("MQ", time.Now().Sub(t), err)
	return err
}
