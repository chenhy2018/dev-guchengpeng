...
type TestServerCfg struct {
	...
	// failure times before echo success
	ReproxyTimes uint64
}
...
func startTestServers(t *testing.T, cfgs []*TestServerCfg) (cli *Client, servers []*httptest.Server, closer func()) {
	...
	for _, c := range cfgs {
		...
		ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			trytime++
			if c.ReproxyTimes > 0 {
				if trytime%(c.ReproxyTimes+1) != 0 {
					httputil.ReplyWithCode(w, 575)
					return
				}
			}
...
func TestClient_PutWithJson(t *testing.T) {
	xl := xlog.NewDummy()

	cli, ts, ts2 := start570serverjson(t)
	para := retstruct{"testtesttest"}
	for i := 0; i < 3; i++ {
		err := cli.PutWithJson(xl, "/", para)
		assert.NoError(t, err)
	}
