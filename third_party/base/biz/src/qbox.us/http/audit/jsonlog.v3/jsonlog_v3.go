...
type Config struct {
	...
	AddReqKeys  []string `json:"add_req_keys"`
	AddRespKeys []string `json:"add_resp_keys"`
}
...
func Open(module string, cfg *Config, acc account.AuthParser) (al *jsonlog.Logger, logf *log.Logger, err error) {
	...
	bdec := jsonlog.BaseDecoder{
		AppendRequestHeaderKeys:  cfg.AddReqKeys,
		AppendResponseHeaderKeys: cfg.AddRespKeys,
	}
	dec = bdec
	if acc != nil {
		if cfg.AuthProxy != 0 {
		...
		dec = &decoder{BaseDecoder: bdec, AuthParser: acc}
	}
