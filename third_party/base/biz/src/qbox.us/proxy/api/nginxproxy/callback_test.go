...
func TestHttpPosts(t *testing.T) {
	...
	for i, URLs := range goodURLs {
		...
		resp, err := ins.Callback(xl, URLs, "www.qiniu.com", "", "abc", access, proto.CallbackConfig{Uid: 0})
	...
	resp, err := ins.Callback(xl, []string{closedUrl}, "www.qiniu.com", "", "abc", access, proto.CallbackConfig{Uid: 0})
