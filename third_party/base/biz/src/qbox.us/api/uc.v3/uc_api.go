...
func (r Service) BucketInfos(l rpc.Logger, zone string) (infos BucketInfos, err error) {
	return r.BucketInfosWithSharedV2(l, zone, tblmgr.RW)
}
...
func (r Service) BucketInfosWithSharedV2(l rpc.Logger, zone string, share_flag int32) (infos BucketInfos, err error) {
...
func (r Service) GlbBucketInfos(l rpc.Logger, region, global string) (infos BucketInfos, err error) {
	return r.GlbBucketInfosWithSharedV2(l, region, global, tblmgr.RW)
}
...
func (r Service) GlbBucketInfosWithSharedV2(l rpc.Logger, region, global string, share_flag int32) (infos BucketInfos, err error) {
...
func (r *Service) ImageWithRetryCodes(l rpc.Logger, bucketName string, srcSiteUrls []string, srcHost string, expires int, qiniuAK, qiniuSK string, retryCodes []int) (err error) {
	url := "/image/" + bucketName
	url += "/from/" + rpc2.EncodeURI(strings.Join(srcSiteUrls, ";"))
	if expires != 0 {
		url += "/expires/" + strconv.Itoa(expires)
	...
	if retryCodes != nil && len(retryCodes) > 0 {
		codeS := ""
		for _, code := range retryCodes {
			codeS += strconv.Itoa(code) + ";"
		}
		url += "/retryCodes/" + rpc2.EncodeURI(codeS)
	}
	return r.Conn.Call(l, nil, url)
}
...
func (r *Service) SetMirrorRawQuerySupport(l rpc.Logger, bucketName string, option string) (err error) {
	params := map[string][]string{
		"bucket": {bucketName},
		"opt":    {option},
	}
	err = r.Conn.CallWithForm(l, nil, r.Host+"/mirrorRawQuerySupport", params)
	return
}

func (r *Service) SetMirrorCheckHeaders(l rpc.Logger, bucketName string, headers []string) (err error) {
	params := map[string][]string{
		"bucket": {bucketName},
		"header": headers,
	}
	err = r.Conn.CallWithForm(l, nil, r.Host+"/mirrorCheckHeaders", params)
	return
}
...
func (r *Service) SetSourceHeaders(l rpc.Logger, bucketName, header, value string) (err error) {
	...
	err = r.Conn.CallWithForm(l, nil, "/sourceHeaders/set", params)
	return
}

func (r *Service) GetSourceHeaders(l rpc.Logger, bucketName string) (sourceHeaders map[string]string, err error) {

	params := map[string][]string{
		"bucket": {bucketName},
	}
	err = r.Conn.CallWithForm(l, &sourceHeaders, "/sourceHeaders/get", params)
	return
}

func (r *Service) DeleteSourceHeaders(l rpc.Logger, bucketName, header string) (err error) {

	params := map[string][]string{
		"bucket": {bucketName},
		"header": {header},
	}
	err = r.Conn.CallWithForm(l, nil, "/sourceHeaders/delete", params)
	return
}

type pfopRuleArgs struct {
	Bucket string `json:"bucket"`
	Name   string `json:"name"`
	uc.PfopRule
}

func (r *Service) SetPfopRules(l rpc.Logger, bucketName, ruleName string, rule uc.PfopRule) (err error) {
	return r.Conn.CallWithJson(l, nil, "/pfopRules/set", pfopRuleArgs{bucketName, ruleName, rule})
}

func (r *Service) GetPfopRules(l rpc.Logger, bucketName string) (rules map[string]uc.PfopRule, err error) {
	err = r.Conn.CallWithJson(l, &rules, "/pfopRules/get", pfopRuleArgs{Bucket: bucketName})
	return
}

func (r *Service) DeletePfopRules(l rpc.Logger, bucketName, ruleName string) (err error) {
	return r.Conn.CallWithJson(l, nil, "/pfopRules/delete", pfopRuleArgs{Bucket: bucketName, Name: ruleName})
}

func (r *Service) SetCorsRules(l rpc.Logger, bucketName string, corsRules []uc.CorsRule) (err error) {
	return r.Conn.CallWithJson(l, nil, "/corsRules/set/"+bucketName, corsRules)
}

func (r *Service) GetCorsRules(l rpc.Logger, bucketName string) (corsRules []uc.CorsRule, err error) {
	err = r.Conn.Call(l, &corsRules, "/corsRules/get/"+bucketName)
	return
}
