...
type BucketInfoWithoutBindDomains struct {
	...
	SourceRetryCodes []int             `json:"source_retry_codes" bson:"source_retry_codes"`
	...
	VersioningEnabled    bool                `json:"versioning_enabled" bson:"versioning_enabled"`
	...
	SourceHeaders        map[string]string   `json:"source_headers" bson:"source_headers"`
	FopAccessWhiteList   []string            `json:"fop_accs_wlist" bson:"fop_accs_wlist,omitempty"`
	MirrorRawQueryOption bool                `json:"mirror_raw_query_option" bson:"mirror_raw_query_option"`
	MirrorCheckHeaders   []string            `json:"mirror_check_headers"` // 检查源站必须带的header，否则认为劫持返回400
	PfopRules            map[string]PfopRule `json:"pfop_rules,omitempty"`
	CorsRules            []CorsRule          `json:"cors_rules, omitempty"`
}

// CNDAutoRefreshRule is the rule applied when file is changed.
// it determine whether files(styled files) need to be auto-refreshed.
type CDNAutoRefreshRule struct {
	CDNAutoRefresh         bool     `json:"cdn_auto_refresh" bson:"cdn_auto_refresh"`
	CDNAutoRefreshStyled   bool     `json:"cdn_auto_refresh_styled"`   // whether cdn auto refresh styled files
	CDNAutoRefreshedStyles []string `json:"cdn_auto_refreshed_styles"` // styled files only with these styles will be auto refreshed
}

// CrosRule means Cross-Origin Resource Sharing rules
type CorsRule struct {
	// 必填；只允许有一个*；*表示全部;
	// "http://abc.com"不能匹配"https://abc.com",也不能匹配"abc.com"
	// "abc.com"不能匹配"http://abc.com"
	AllowedOrigins []string `json:"allowed_origin" bson:"allowed_origin"`
	AllowedMethods []string `json:"allowed_method" bson:"allowed_method"` // 必填；不支持*；
	AllowedHeaders []string `json:"allowed_header" bson:"allowed_header"` // 选填；空则不允许任何header。*代表允许任何header；其他*不生效
	ExposedHeaders []string `json:"exposed_header" json:"exposed_header"` // 选填；*不生效；X-Log, X-Reqid是默认会暴露的两个header；其他的header如果没有设置，则不会暴露；
	MaxAge         int      `json:"max_age" json:"max_age"`               // in seconds；选填；空代表0
}

type PfopRule struct {
	Prefix              string   `json:"prefix" bson:"prefix"`
	MimeTypes           []string `json:"mime_types" bson:"mime_types"`
	PersistentOps       string   `json:"persistent_ops" bson:"persistent_ops"`
	PersistentNotifyUrl string   `json:"persistent_notify_url" bson:"persistent_notify_url"`
	PersistentPipeline  string   `json:"persistent_pipeline" bson:"persistent_pipeline"`
}
...
func New(host string, t http.RoundTripper) *Service {
	...
	return &Service{Host: host, Conn: rpc.Client{Client: client}}
}
...
func (r *Service) CDNAutoRefresh(bucket string, refresh, refreshStyled int, refreshedStyles string) (code int, err error) {

	params := map[string][]string{
		"bucket":          {bucket},
		"refresh":         {strconv.Itoa(refresh)},
		"refreshStyled":   {strconv.Itoa(refreshStyled)},
		"refreshedStyles": {refreshedStyles},
	}
	code, err = r.Conn.CallWithForm(nil, r.Host+"/cdnAutoRefresh", params)
	return
}
