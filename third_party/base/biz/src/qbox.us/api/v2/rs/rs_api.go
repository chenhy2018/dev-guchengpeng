...
func (rs Service) GetVersion(entryURI, version, attName string) (data GetRet, code int, err error) {
...
func (rs Service) StatVersion(entryURI, version string) (entry Entry, code int, err error) {
	code, err = rs.Conn.Call(&entry, RS_HOST+"/stat/"+rpc.EncodeURI(entryURI)+"/version/"+version)
	return
}

func (rs Service) StatAllVersions(entryURI string) (entries []Entry, code int, err error) {
	response := struct {
		Entries []Entry `json:"entries"`
	}{
		Entries: entries,
	}
	code, err = rs.Conn.Call(&response, RS_HOST+"/stat/"+rpc.EncodeURI(entryURI)+"/allVersions/1")
	return response.Entries, code, err
}
...
func (rs Service) DeleteWithVersion(entryURI string) (ret VersionRet, code int, err error) {
	code, err = rs.Conn.Call(&ret, RS_HOST+"/delete/"+rpc.EncodeURI(entryURI))
	return
}

func (rs Service) DeleteVersion(entryURI, version string) (code int, err error) {
	return rs.Conn.Call(nil, RS_HOST+"/delete/"+rpc.EncodeURI(entryURI)+"/version/"+version)
}
...
func (rs Service) MoveWithVersion(entryURISrc, entryURIDest string) (ret VersionRet, code int, err error) {
	code, err = rs.Conn.Call(&ret, RS_HOST+"/move/"+rpc.EncodeURI(entryURISrc)+"/"+rpc.EncodeURI(entryURIDest))
	return
}

func (rs Service) MoveVersion(entryURISrc, versionSrc, entryURIDest string) (ret VersionRet, code int, err error) {
	code, err = rs.Conn.Call(&ret, RS_HOST+"/move/"+rpc.EncodeURI(entryURISrc)+"/"+rpc.EncodeURI(entryURIDest)+"/fromVersion/"+versionSrc)
	return
}
...
func (rs Service) CopyWithVersion(entryURISrc, entryURIDest string) (ret VersionRet, code int, err error) {
	code, err = rs.Conn.Call(&ret, RS_HOST+"/copy/"+rpc.EncodeURI(entryURISrc)+"/"+rpc.EncodeURI(entryURIDest))
	return
}

func (rs Service) CopyVersion(entryURISrc, versionSrc, entryURIDest string) (ret VersionRet, code int, err error) {
	code, err = rs.Conn.Call(&ret, RS_HOST+"/copy/"+rpc.EncodeURI(entryURISrc)+"/"+rpc.EncodeURI(entryURIDest)+"/fromVersion/"+versionSrc)
	return
}
...
func (rs Service) ChgmVersion(entryURI, version, mimeType string) (code int, err error) {
	return rs.Conn.Call(nil, RS_HOST+"/chgm/"+rpc.EncodeURI(entryURI)+"/mime/"+rpc.EncodeURI(mimeType)+"/version/"+version)
}

func (rs Service) ChgmWithCond(entryURI, mimeType string, cond map[string]interface{}) (code int, err error) {
	u := RS_HOST + "/chgm/" + rpc.EncodeURI(entryURI) + "/mime/" + rpc.EncodeURI(mimeType)
	var condStr string
	for k, v := range cond {
		condStr += fmt.Sprintf("%v=%v&", k, v)
	}
	if strings.HasSuffix(condStr, "&") {
		u += "/cond/" + rpc.EncodeURI(condStr[:len(condStr)-1])
	}
...
func (rs Service) ChVersionType(entryURI, version string, Type FileType) (code int, err error) {
	return rs.Conn.Call(nil, RS_HOST+"/chtype/"+rpc.EncodeURI(entryURI)+"/type/"+fmt.Sprint(Type)+"/version/"+version)
}

func (rs Service) ChStatus(entryURI string, status uint32, cond map[string]interface{}) (code int, err error) {
	u := RS_HOST + "/chstatus/" + rpc.EncodeURI(entryURI) + "/status/" + fmt.Sprintf("%d", status)
...
func (rs Service) ChMetaWithCond(entryURI, metaKey, metaValue string, cond map[string]interface{}) (code int, err error) {
	u := RS_HOST + "/chgm/" + rpc.EncodeURI(entryURI) + "/x-qn-meta-" + metaKey + "/" + rpc.EncodeURI(metaValue)
	var condStr string
	for k, v := range cond {
		condStr += fmt.Sprintf("%v=%v&", k, v)
	}
...
func (rs Service) MkVersioningbucket(bucketName, zone string) (code int, err error) {
	url := RS_HOST + "/mkbucket/" + bucketName
	if zone != "" {
		url += "/zone/" + zone
	}
