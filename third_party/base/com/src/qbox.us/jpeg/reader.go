...
func (d *decoder) processSOF(n int, configOnly bool) error {
	...
	case 6 + 3*nCMYKComponent:
		d.nComp = nCMYKComponent
	default:
	...
	if d.nComp > 3 {
		return UnsupportedError("jpeg image components more than 3")
	}
	for i := 0; i < d.nComp; i++ {
...
func (d *decoder) processDQT(n int, ctx *qualityCtx) error {
...
func (d *decoder) decode(r io.Reader, configOnly bool) (image.Image, int, error) {
	...
	if d.tmp[0] != 0xff || d.tmp[1] != soiMarker {
		return nil, 0, FormatError("missing SOI marker")
	}
	...
	for {
		...
		if n < 0 {
			return nil, 0, FormatError("short segment length")
		}
		...
		case marker == sof0Marker || marker == sof2Marker: // Start Of Frame.
			...
			if configOnly {
				return nil, qctx.result(), err
			}
		case marker == dhtMarker: // Define Huffman Table.
			err = d.processDHT(n)
		case marker == dqtMarker: // Define Quantization Table.
			err = d.processDQT(n, &qctx)
	...
	if d.img1 != nil {
		return d.img1, qctx.result(), nil
	}
	if d.img3 != nil {
		return d.img3, qctx.result(), nil
	}
	return nil, 0, FormatError("missing SOS marker")
}
...
type decoderExtender interface {
	DecoderExtend() (r io.Reader, quality *int)
}
...
func DecodeConfig(r io.Reader) (cfg image.Config, err error) {
	var quality int
	pq := &quality
	if ex, ok := r.(decoderExtender); ok {
		r, pq = ex.DecoderExtend()
	}
	var d decoder
	if _, *pq, err = d.decode(r, true); err != nil {
