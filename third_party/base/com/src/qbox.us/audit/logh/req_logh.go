...
import (
	...
	"qbox.us/cc"
	hbytes "qbox.us/net/httputil/bytes"
	"qbox.us/servestk"
)

// ----------------------------------------------------------
// NOTE: 这个包已经迁移到 github.com/qiniu/http/audit/jsonlog
...
func (r BaseDecoder) DecodeRequest(req *http.Request) (url_ string, h, params M) {
	...
	set(h, req.Header, "X-Src")

	url_ = req.URL.Path
	if ok {
		switch ct[0] {
		case "application/x-www-form-urlencoded":
			seekable, err := hbytes.Seekable(req)
			if err == nil {
				req.ParseForm()
...
// NOTE: 这个包已经迁移到 github.com/qiniu/http/audit/jsonlog
//
func New(mod string, l Logger, dec Decoder, limit int) *Instance {
	if dec == nil {
		dec = DefaultDecoder
	}
	return &Instance{l, dec, mod, limit, true}
}

// NOTE: 这个包已经迁移到 github.com/qiniu/http/audit/jsonlog
//
func NewEx(mod string, l Logger, dec Decoder, limit int, xlog bool) *Instance {
	if dec == nil {
		dec = DefaultDecoder
	}
	return &Instance{l, dec, mod, limit, xlog}
}
...
func (r *Instance) handle(w http.ResponseWriter, req *http.Request, f func(w http.ResponseWriter, req *http.Request)) {
	...
	body := cc.NewBytesWriter(make([]byte, r.limit))
...
func (r *Instance) SetLogger(l *log.Logger) {
	// nothing todo
}

func (r *Instance) Handler() func(w http.ResponseWriter, req *http.Request, f func(w http.ResponseWriter, req *http.Request)) {
