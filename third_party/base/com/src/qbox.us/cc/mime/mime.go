...
// 用户自定义的meta key 支持 字母、数字、下划线、减号、感叹号，长度小于等于50
func IsValidMetaKey(key string) bool {
	return len(key) <= 50+len("x-qn-meta-") && IsType(XMLSYMBOL_NEXT_CHAR|EXCLAMA, key)
}

//相比ValidMimeType可以出现[ ],[:],["]
func IsValidMetaValue(value string) bool {
	if value == "" {
		return true
	}
	return len(value) <= 200 && IsType(validMimeCtype|COLON|SPACE_BAR|QUOT, value)
}
