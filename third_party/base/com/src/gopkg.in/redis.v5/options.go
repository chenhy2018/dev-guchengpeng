...
import (
	...
	qiniulog "github.com/qiniu/log.v1"
...
func (opt *Options) initWithProxy(lbs *lbsocketproxy.LbSocketProxy) {
	opt.init()
	if lbs != nil {
		opt.Dialer = func() (net.Conn, error) {
			addr, err := resolveAddr(opt.Addr)
			if err != nil {
				panic(err)
			}
			qiniulog.Debug("redis use LbSocketProxy")
			return lbs.Dial(addr)
		}
	}
}
...
func resolveAddr(addr string) (*net.TCPAddr, error) {
	...
	if err != nil {
		qiniulog.Error("SYNC Failed to resolve ", addr, ": ", err.Error())
		return nil, err
	}
	if tcpaddr.String() != addr {
		qiniulog.Warn("SYNC Address ", addr, " resolved as ", tcpaddr.String())
	}
	return tcpaddr, nil
}
