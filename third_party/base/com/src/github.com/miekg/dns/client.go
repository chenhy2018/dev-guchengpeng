...
//
//	c := new(dns.Client)
//	in, rtt, err := c.Exchange(message, "127.0.0.1:53","127.0.0.1:7890")
//
// Exchange does not retry a failed query, nor will it fall back to TCP in
// case of truncation.
//
// local: the local address to use when dialing an address.
func (c *Client) ExchangeWithLocalAddr(m *Msg, a string, local string) (r *Msg, rtt time.Duration, err error) {
	if !c.SingleInflight {
		return c.exchange(m, a, local)
	}
	// This adds a bunch of garbage, TODO(miek).
	t := "nop"
	if t1, ok := TypeToString[m.Question[0].Qtype]; ok {
...
func (c *Client) exchange(m *Msg, a string, local string) (r *Msg, rtt time.Duration, err error) {
	timeout := dnsTimeout
	var co *Conn
	if c.DialTimeout != 0 {
	...
	if c.Net == "" {
		co, err = DialTimeout("udp", a, local, timeout)
	} else {
		co, err = DialTimeout(c.Net, a, local, timeout)
	}
	if err != nil {
		return nil, 0, err
	}
...
func DialTimeout(network, address, local string, timeout time.Duration) (conn *Conn, err error) {
	conn = new(Conn)
