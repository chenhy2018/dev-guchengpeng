...
func SetHeaders(h http.Header, meta *Metas, req *http.Request) {
	...
	if meta.DispositionType != "" {
		...
		h.Set("Content-Disposition", meta.DispositionType+"; filename=\""+meta.FileName+"\"; filename*=utf-8' '"+url.PathEscape(meta.FileName))
	}
	...
	if ims := req.Header.Get("If-Modified-Since"); ims != "" {
		if t, err := http.ParseTime(ims); err == nil {
			if meta.LastModified.Truncate(time.Second).After(t) {
				// IMS is in second, so meta.LastModified must be truncated before comparing.
				return false
			} else {
				if304 = true
			}
...
func (w *headerResponseWriter) Done(err error) {
	if w.firstWrite {
		if err != nil {
			w.Header().Add("X-KODO-RESPCODE", strconv.Itoa(httputil.DetectCode(err)))
		}
		w.ResponseWriter.WriteHeader(w.headerCode)
	}
	return
