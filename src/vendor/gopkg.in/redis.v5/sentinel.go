...
func (net *netAddr) Network() string {
	return "tcp"
}

func (net *netAddr) String() string {
	return net.Addr
}

func NewFailoverClientWithProxy(failoverOpt *FailoverOptions, lbs *lbsocketproxy.LbSocketProxy) *Client {
	opt := failoverOpt.options()
	opt.initWithProxy(lbs)
	failover := &sentinelFailover{
		masterName:    failoverOpt.MasterName,
		sentinelAddrs: failoverOpt.SentinelAddrs,
