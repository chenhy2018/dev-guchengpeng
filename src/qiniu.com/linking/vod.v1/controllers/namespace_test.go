...
import (
	...
	"qiniu.com/linking/vod.v1/system"
)
...
var (
	...
	user     = userInfo{
		uid: "138008",
		ak:  "JAwTPb8dmrbiwt89Eaxa4VsL4_xSIYJoJh4rxxx",
		sk:  "G5mtjT3QzG4Lf7jpCAN5Pxxxxx",
	}
)
...
func TestUpdateNamespace(t *testing.T) {
	...
	c.Params = append(c.Params, param)
	c.Request = req
	monkey.Patch(checkBucketInKodo, func(bucket string, user *userInfo) error { return errors.New("inavailable bucket") })
	UpdateNamespace(c)
	assert.Equal(t, c.Writer.Status(), 400, c.Writer.Header().Get("x-ReqId"))
...
func TestGetNameSpaceInfo(t *testing.T) {
	...
	c.Request = req
	RegisterNamespace(c)
	err, _ = GetNameSpaceInfo(xl, "ipcamera", "test1a", user.uid)
